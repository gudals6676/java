...
public class StudentUpdate {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					StudentUpdate window = new StudentUpdate(vo);
	 ...
	 * @param vo 
	 */
	public StudentUpdate(StudentVO r_vo) {
		vo = r_vo;
		System.out.println("내 정보 id >> " + vo.getStu_id());
		initialize();
	...
	private void initialize() {
		...
		btn_update.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent arg0) {
				...
				StudentVO u_vo = new StudentVO(vo.getStu_id(),pw,name,birth,tel,addr,email);
		...
		StudentVO myInfo = dao.StudentUpdate(vo.getStu_id());
