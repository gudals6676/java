...
public class P2_4Teacher_GUI_1 {
	...
	public void initialize() {
		...
		stu_id.setHorizontalAlignment(SwingConstants.LEFT);
		panel_1.add(stu_id);
		...
		String[] header = { "학생_id", "담임선생님_id", "반번호", "비밀번호", "이름", "생일", "전화번호", "주소지", "이메일", "면담", };
		// 커럼값(데이터) -2차원 배열
		...
		btnNewButton.addActionListener(new ActionListener() {// 점수조회
			public void actionPerformed(ActionEvent e) {
				...
				frame.dispose();
				// 성적을 조회하는부분을 불러오는 생성자
			}
		});
		btnNewButton.setBounds(490, 410, 182, 23);
		...
		textField.setBounds(490, 346, 83, 21);
		frame.getContentPane().add(textField);
		textField.setColumns(10);

		// 학생 정보의 이름을 매개체로 정보를 검색해서 면담신청을함
		JButton btnNewButton_1 = new JButton("\uBA74\uB2F4 \uC2E0\uCCAD");
		btnNewButton_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				String name = textField.getText();
				if (name == null) {
					JOptionPane.showMessageDialog(null, "입력값을 넣어주세요","입력값 오류",JOptionPane.ERROR_MESSAGE);
				} else {
		...
		btnNewButton_1.setBounds(585, 346, 87, 23);
		frame.getContentPane().add(btnNewButton_1);

		textField_1 = new JTextField();
		textField_1.setBounds(490, 379, 83, 21);
		...
		btnNewButton_2.setBounds(585, 377, 87, 23);
